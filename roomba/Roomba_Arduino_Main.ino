...
void loop() {
  ...
  if (command.substring(0) == "up\n") {
    stepperUp();
  }
  if (command.substring(0) == "down\n") {
    stepperDown();
  } 

  if (command.substring(0) == "turn left\n") {
    
  }

  if (command.substring(0) == "check pallets\n") {
    Serial.println("Checking the pallets");
    bool codeScanResult;
